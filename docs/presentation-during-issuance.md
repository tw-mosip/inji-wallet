...
classDiagram
  ...
  class PresentationDuringIssuanceHandler {
    + constructor(selectCredentialsForPresentation: (ovpRequest: AuthorizationRequest) -> Map\<String, Map\<FormatType, List\<Any>>>, signVerifiablePresentation: (payload: Map<FormatType, UnsignedVPToken>) -> Map<FormatType, VPTokenSigningResult>)
    ...
    - validatePresentationRequest(authorizationRequest: Map<String,Any>) : AuthorizationRequest
...
- Requirements of the Presentation interaction from Wallet (got as constructor params)
  ...
  - `selectCredentialsForPresentation` callback
    - input: presentation request | output: Map<String, Map<FormatType, List<Any>>> holding selected credentials - map of {credential_type: {format_type: list of credentials}}
    - selectCredentialsForPresentation: (ovpRequest: AuthorizationRequest) -> Map<String, Map<FormatType, List<Any>>>
...
- Methods
  ...
  - validatePresentationRequest(authorizationRequest: Map<String,Any>) : AuthorizationRequest [private]
...
| 2                                                            | Displaying the presentation request to the user and obtaining user consent            | inji-vci-client | wallet (callback)         | - inji-vci-client does a callback to wallet passing the VP request and getting the matching credentials selected by user (selectedCredentials) (Map<String, Map<FormatType, List<Any>>>) in response.<br/>- The user consent are all handled by the callback. Basically step no 2,3,4,5 are handled and the matching credentials are returned or error for consent rejection is thrown<br/>- Method used: selectCredentialsForPresentation (wallet)<br/>- sample: `selectCredentialsForPresentation(ovpRequest: AuthorizationRequest) : Map<String, Map<FormatType, List<Any>>>` |
