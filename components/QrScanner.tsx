...
import { BarCodeEvent, BarCodeScanner, BarCodeScannerResult } from 'expo-barcode-scanner';
...
export const QrScanner: React.FC<QrScannerProps> = (props) => {
  ...
  const [showCameraPermissionDeniedBanner, setShowCameraPermissionDeniedBanner] = useState(false);
  ...
  useEffect(() => {
    if (isActive && !Boolean(hasCameraPermission)) {
      (async () => {
        ...
        const cameraPermissionResult = await BarCodeScanner.requestPermissionsAsync();
