...
export function useSendVPScreen() {
  ...
  const noCredentialsMatchingVPRequest =
    ...
    const isOVPViaDeepLink = useSelector(openID4VPService, selectIsOVPViaDeeplink);

    const getAdditionalMessage = () => {
      return isOVPViaDeepLink && isIOS()
        ? t('errors.additionalMessage')
        : '';
    };
  ...
  useEffect(() => {
    if (noCredentialsMatchingVPRequest && !hasLoggedErrorRef.current) {
      ...
      generateAndStoreLogMessage('NO_CREDENTIAL_MATCHING_REQUEST', claimsAsString);
    ...
    } else if (error.includes('credential mismatch detected') && !hasLoggedErrorRef.current) {
      setErrorModalData({
        show: true,
      ...
      generateAndStoreLogMessage('CREDENTIAL_MISMATCH_FROM_KEBAB', claimsAsString);
      hasLoggedErrorRef.current = true;
  
    } else if (error.includes('none of the selected VC has image') && !hasLoggedErrorRef.current) {
      setErrorModalData({
        show: true,
    ...
    } else if (error.startsWith('vc validation') && !hasLoggedErrorRef.current) {
      setErrorModalData({
        show: true,
